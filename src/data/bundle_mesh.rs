...
impl ColorGrid {
    ...
    fn sample_color(&self, gx: f32, gy: f32, gz: f32) -> [f32; 4] {
        ...
        let c = lerp(
            lerp(
                lerp(self.voxel_color(x0, y0, z0), self.voxel_color(x1, y0, z0), fx),
                lerp(self.voxel_color(x0, y1, z0), self.voxel_color(x1, y1, z0), fx),
                fy,
            ),
            lerp(
                lerp(self.voxel_color(x0, y0, z1), self.voxel_color(x1, y0, z1), fx),
                lerp(self.voxel_color(x0, y1, z1), self.voxel_color(x1, y1, z1), fx),
                fy,
            ),
            fz,
        );
        ...
        for iy in 0..ny {
            for ix in 0..nx {
                ...
                for (ki, &k) in kernel.iter().enumerate() {
                    let sx = (ix as isize + ki as isize - radius as isize)
    ...
    for iz in 0..nz {
        for iy in 0..ny {
            for ix in 0..nx {
                let mut val = 0.0f32;
                for (ki, &k) in kernel.iter().enumerate() {
                    let sy = (iy as isize + ki as isize - radius as isize)
    ...
    for iz in 0..nz {
        for iy in 0..ny {
            for ix in 0..nx {
                let mut val = 0.0f32;
                for (ki, &k) in kernel.iter().enumerate() {
                    let sz = (iz as isize + ki as isize - radius as isize)
...
fn largest_component(vertices: &[BundleMeshVertex], indices: &[u32]) -> Vec<u32> {
    ...
    let best = comp_sizes.iter().enumerate()
        .max_by_key(|&(_, &s)| s)
        .map(|(i, _)| i as u32)
        .unwrap_or(0);
...
pub fn build_bundle_mesh(
    ...
    let mc_threshold = if smooth_sigma >= 0.5 {
        ...
        let k_sum: f32 = (0..size)
            .map(|i| { let x = i as f32 - radius as f32; (-0.5 * x * x / (smooth_sigma * smooth_sigma)).exp() })
            .sum();
    ...
    let vertices: Vec<BundleMeshVertex> = mesh.vertices.iter().map(|v| {
        let wx = v.posit.x;
        let wy = v.posit.y;
        let wz = v.posit.z;
        let gx = (wx - mn[0]) / vs;
        ...
        BundleMeshVertex {
            ...
            color:    grid.sample_color(gx, gy, gz),
        }
    }).collect();
